...
const CONFIG = Object.freeze({
  ...
  // Phạm vi tìm meteor quanh nhân vật.
  autoSwordTargetRadius: 60,
  ...
  // Tốc độ kiếm đang giữ lượt quay về.
  autoSwordReturnSpeed: 34,

  // Vị trí chờ của kiếm đang giữ lượt,
  // tính theo hệ tọa độ cục bộ của player.
  autoSwordReadyOffsetX: 0,
  autoSwordReadyOffsetY: 0.55,
  autoSwordReadyOffsetZ: 2,

  // Khoảng cách đủ gần để xem là
  // kiếm đã về đúng vị trí chờ.
  autoSwordReadyArrivalDistance: 0.08,

  // Kích thước bình thường của kiếm tự động.
  autoSwordScale: 0.4,

  // Hệ số phóng lớn của kiếm đang giữ lượt.
  // Kích thước khi tấn công: 0.4 × 1.35 = 0.54.
  autoSwordActiveScaleMultiplier: 1,

  // Tốc độ chuyển đổi giữa hai kích thước.
  // Giá trị càng lớn thì phóng lớn và thu nhỏ càng nhanh.
  autoSwordScaleFollowStrength: 10,

  // Mỗi kiếm được chém tối đa 10 meteor
  // trong một lượt hoạt động.
  autoSwordKillsPerTurn: 10,
...
/*
 * Điểm xuất phát của đạn, được gắn trực tiếp
 * vào vị trí mũi kiếm đang được nhân vật giữ.
 */
let aimMuzzle = null;
 ...
 * Vị trí chờ của kiếm đang giữ lượt.
 */
const autoSwordReadyPosition = new THREE.Vector3();
...
function setupPlayer(characterAsset) {
  ...
  /*
   * Cập nhật ma trận trước khi tính
   * bounding box của kiếm.
   */
  player.updateMatrixWorld(true);

  /*
   * Tạo điểm bắn tại chính mũi kiếm.
   *
   * Kiếm được quy ước hướng theo local -Z,
   * vì vậy mũi kiếm nằm ở bounds.min.z.
   */
  aimMuzzle =
    new THREE.Object3D();
  ...
  if (!heldSwordBounds.isEmpty()) {
    /*
     * Bắt đầu từ chính giữa bounding box
     * theo phương X và Y.
     */
    ...
    /*
     * Chuyển tọa độ thế giới về tọa độ
     * cục bộ của aimSwordPivot.
     */
    ...
    /*
     * Vị trí dự phòng nếu model không
     * tạo được bounding box hợp lệ.
     */
  ...
  aimSwordPivot.add(aimMuzzle);

  /*
   * Áp dụng vị trí và kích thước responsive.
   */
...
function getMuzzlePosition(target) {
  /*
   * Lấy trực tiếp vị trí thế giới
   * của mũi kiếm.
   *
   * Vì aimMuzzle là con của aimSwordPivot,
   * vị trí này tự động thay đổi khi:
   *
   * - Nhân vật xoay trái/phải.
   * - Nhân vật nghiêng lên/xuống.
   * - Player thay đổi position.
   * - Player thay đổi scale.
   */
  if (aimMuzzle) {
    return aimMuzzle.getWorldPosition(
      target
    );
  }

  /*
   * Giá trị dự phòng nếu model kiếm
   * chưa được tạo xong.
   */
  target.set(
    0,
  ...
  if (meteor) {
    ...
    if (
      ...
      */
      ...
      /*
      * Nếu đây là kiếm đang giữ lượt,
      * nó phải quay về cạnh nhân vật
      * trước khi chọn meteor mới.
      */
      if (
        autoSwordOwner ===
...
function enterAutonomousSwordOrbit(sword) {
  ...
  sword.userData.isReturning = false;
}

/*
 * Lấy vị trí chờ của kiếm đang giữ lượt.
 *
 * Dùng player.localToWorld() để vị trí
 * tự thích ứng theo position và scale
 * hiện tại của player.
 */
function getAutonomousSwordReadyPosition(
  target
) {
  target.set(
    CONFIG.autoSwordReadyOffsetX,
    CONFIG.autoSwordReadyOffsetY,
    CONFIG.autoSwordReadyOffsetZ
  );
...
function setAutonomousSwordGolden(
  ...
  /*
  * Khi kiếm hết lượt, tắt trail.
  * Khi nhận lượt, activateAutonomousSword()
  * sẽ bật trail ngay.
  */
  if (
...
function createAutonomousSword(swordIndex) {
   ...
   */
  sword.userData.isQueueActive =
    false;

  /*
   * true khi kiếm đang bay trở về
   * vị trí chờ cạnh nhân vật.
   */
  sword.userData.isReturning =
    false;

  /*
  * Kiếm mới chưa đến lượt sẽ bay quanh.
  */
  sword.userData.isOrbiting =
    true;
  sword.add(visual);
...
function activateAutonomousSword(sword) {
  ...
  /*
   * Tắt quyền tấn công và hiệu ứng
   * của tất cả kiếm khác.
   */
  for (
  ...
  sword.userData.isReturning = true;
  sword.userData.turnKills = 0;

  /*
   * Bật màu vàng, emissive và đèn
   * ngay khi kiếm nhận lượt.
   */
  setAutonomousSwordGolden(
    sword,
    true
  ...
  /*
   * Bật trail ngay, không còn delay.
   */
  ...
  /*
   * Kiếm phải về cạnh nhân vật trước
   * khi tìm mục tiêu đầu tiên.
   */
}
...
function completeAutonomousSwordTurn(
  sword
) {
  /*
   * Hàm này chỉ được gọi sau khi:
   *
   * - Kiếm đã chém đủ 10 meteor.
   * - Kiếm đã quay về cạnh nhân vật.
   */
  ...
  /*
   * Tắt hiệu ứng của kiếm vừa hết lượt.
   */
  setAutonomousSwordGolden(
    sword,
    false
...
function findFarthestAvailableMeteor() {
  /*
  ...
  let farthestMeteor = null;

  let farthestDistanceSquared =
    -Infinity;

  /*
   * Chỉ xét meteor nằm trong phạm vi
   * hoạt động của kiếm tự động.
   */
  ...
  for (const meteor of meteors) {
    ...
    /*
     * Chọn meteor có khoảng cách
     * lớn nhất so với nhân vật.
     */
    if (
      distanceSquared >
      ...
      farthestMeteor = meteor;
    }
  }

  return farthestMeteor;
}
...
function updateAutonomousSwords(delta) {
  ...
  /*
   * Chỉ kiếm đầu hàng đợi được tấn công.
   */
  for (const sword of activeSwords) {
    ...
    /*
     * Kiếm không giữ lượt không được
     * giữ bất kỳ meteor nào.
     */
  ...
  /*
   * Nếu target biến mất khi kiếm đang bay,
   * kiếm phải quay về cạnh nhân vật.
   */
  ...
  /*
   * Những kiếm không giữ lượt
   * tiếp tục quay quanh nhân vật.
   */
  ...
  for (
    const sword of activeSwords
  ) {
    const isActiveSword =
      sword === activeAutonomousSword;

    /*
     * Kiếm giữ lượt phóng lớn ngay.
     */
    const targetSwordScale =
      CONFIG.autoSwordScale *
      (
        isActiveSword
          ? CONFIG
              .autoSwordActiveScaleMultiplier
          : 1
      );
    ...
    const scaleFollowAmount =
      ...
      Math.exp(
        -CONFIG
          .autoSwordScaleFollowStrength *
          delta
      );

    const nextSwordScale =
    ...
    /*
     * Kiếm không giữ lượt:
     * quay đều quanh nhân vật.
     */
    if (!isActiveSword) {
      const orbitIndex =
        orbitingSwords.indexOf(
          sword
        );
      ...
      /*
       * Mũi kiếm hướng ra ngoài vòng tròn.
       */
      autoSwordDirection
        .copy(sword.position)
    ...
    /*
     * Kiếm đang quay về cạnh nhân vật.
     */
    if (
      sword.userData.isReturning
    ) {
      getAutonomousSwordReadyPosition(
        autoSwordReadyPosition
      );

      autoSwordDirection
        .copy(autoSwordReadyPosition)
        .sub(sword.position);

      const returnDistance =
        autoSwordDirection.length();

      const returnTravelDistance =
        CONFIG.autoSwordReturnSpeed *
        delta;

      /*
       * Kiếm đã về đủ gần vị trí chờ.
       */
      if (
        returnDistance <=
        CONFIG
          .autoSwordReadyArrivalDistance +
          returnTravelDistance
      ) {
        sword.position.copy(
          autoSwordReadyPosition
        );
        ...
        /*
         * Sau nhát thứ 10, kiếm phải
         * về vị trí chờ rồi mới hết lượt.
         */
        if (
      ...
      /*
       * Khi đang quay về, mũi kiếm
       * hướng theo hướng chuyển động.
       */
      if (
        returnDistance > 0.0001
      ) {
        autoSwordDirection
          .divideScalar(
            returnDistance
